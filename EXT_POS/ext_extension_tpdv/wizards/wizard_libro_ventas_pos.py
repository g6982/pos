...
class libro_ventas(models.TransientModel):
    ...
    state = fields.Selection([('choose', 'choose'), ('get', 'get')],default='choose',invisible=True) ##Genera los botones de exportar xls y pdf como tambien el de cancelar
    report = fields.Binary('Prepared file', filters='.xls', readonly=True,invisible=True)
    name = fields.Char('File Name', size=32,invisible=True)
    company_id = fields.Many2one('res.company','Company',default=lambda self: self.env.company.id,invisible=True)

    line  = fields.Many2many(comodel_name='pos.wizard.pdf.ventas', string='Lineas',invisible=True)
