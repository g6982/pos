        ...
        class ReceiptScreen extends AbstractReceiptScreen {
            ...
            async ImprimirFiscal () {
                ...
                for (var i = 0; i < order.orderlines.models.length; i += 1) {
                    ...
                    var product = order.orderlines.models[i].product.default_code ? order.orderlines.models[i].product.default_code.replace('&', '') + " " + order.orderlines.models[i].product.display_name.replace('&', '') : order.orderlines.models[i].product.display_name.replace('&', '');
                    var precio = 0;
                    if (order.orderlines.models[i].discount == 0){
                        precio = order.orderlines.models[i].price
                    } else {
                        var descuento = (order.orderlines.models[i].discount / 100) * order.orderlines.models[i].price
                        precio = order.orderlines.models[i].price - descuento
                    }
                    lineas.push({
                        'product': product.slice(0,57),
                        'cantidad': order.orderlines.models[i].quantity,
                        'precio': precio,
                        'impuesto': valor_impuesto,
                        'descuento': 0 //order.orderlines.models[i].discount
                    })
                }
