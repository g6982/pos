...
class AccountMove(models.Model):
    ...
    def tax_aliquot_general(self, ret):
        ...
        count_move = len(self.invoice_line_ids)
        count_ret = 0
        for move in self.invoice_line_ids:
            count_move -= 1
            for tax in move.tax_ids.filtered(lambda x: x.aliquot == 'general'):
                count_ret += 1
                base += move.price_subtotal
                amount_total += move.price_total
                amount_iva += (amount_total - base)
                amount_detained += (amount_iva * rate) / 100
                if count_ret > count_move:
                    ret.write({
                        'line_ids': [(0, 0, {
                            'retention_iva_id': ret.id,
                            'amount_tax': self.amount_rate_retention_iva(amount_iva),
                            'amount_retention': self.amount_rate_retention_iva(amount_detained),
                            'retention_rate': rate,
                            'base': base,
    ...
    def tax_aliquot_reduced(self, ret):
        ...
        for move in self.invoice_line_ids:
            count_move -= 1
            for tax in move.tax_ids.filtered(lambda x: x.aliquot == 'reduced'):
                count_ret += 1
                base += move.price_subtotal
                amount_total += move.price_total
                amount_iva += (amount_total - base)
    ...
    def tax_aliquot_additional(self, ret):
        ...
        for move in self.invoice_line_ids:
            count_move -= 1
            for tax in move.tax_ids.filtered(lambda x: x.aliquot == 'additional'):
                count_ret += 1
                base += move.price_subtotal
                amount_total += move.price_total
                amount_iva += (amount_total - base)
