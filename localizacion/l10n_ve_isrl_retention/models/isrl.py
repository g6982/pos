...
class IsrlRetention(models.Model):
    ...
    def action_post(self):
        line = []
        # amount = self.amount_retention_total()
        ...
        if self.move_type in ['in_invoice', 'in_refund']:
            balance_credit += zero - amount
            balance_debit += amount - zero
        if self.move_type in ['out_invoice', 'out_receipt']:
            balance_credit += amount - zero
            balance_debit += zero - amount
        ...
        if self.move_type in ['out_invoice', 'out_refund']:
            line.append((0, 0, {
                ...
                'debit': 0.0,
                'balance': -amount,
                'price_unit': balance_debit,
                'price_subtotal': balance_debit,
                'price_total': balance_debit
            ...
            line.append((0, 0, {
                ...
                'credit': 0.0,
                'balance': amount,
                'price_unit': balance_credit,
                'price_subtotal': balance_credit,
                'price_total': balance_credit
            }))
    ...
    def action_partial_reconcile(self, move):
        ...
        amount_credit = 0.0
        total = 0.0
        move_obj = self.env['account.move'].search([('retention_id', '=', self.id), ('move_type', '=', self.move_type)])
        ...
        vals.update({
            ...
            self.move_type in ('out_invoice', 'out_receipt') else total,
