    ...
    def pre_retention(self):
        if not self.retention_id:
            ...
            if self.move_type not in ['entry']  \
                ...
                for move in self.invoice_line_ids:
                    rate = self.amount_rate_retention(move.price_subtotal)
                    for r in move.rate_ids:
                        if self.partner_id.people_type == r.people_type and rate > r.min:
                            ...
                            retention_line_obj.create({
                                ...
                                'base': self.amount_rate_retention(base),
                                'qty_retention': self.amount_rate_retention(subtotal),
                                'subtracting': r.subtract,
                                'total': self.amount_rate_retention(total)
                            })
